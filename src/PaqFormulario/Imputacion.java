 ...
 * @author Danny Veron
 */
public final class Imputacion extends javax.swing.JDialog {
     ...
     */
    public Imputacion(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
        botonEnter();
        centrar();
    ...
    private void ActualizarGrilla() {
        ...
        try {
            ...
            vaciarTabla();
            String sql = "SELECT *"
                    + " FROM imputacion where estado is null"
                    + " ORDER BY imp_des";
            ...
            Logger.getLogger(Imputacion.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void btnAgrActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgrActionPerformed
        try {
            ...
            String sql = "Select ifnull(max(imp_cod),0)+1 codigo From imputacion";
            //System.out.println(sql);
            conexion();
            rs = con.Consulta(sql);
            rs.next();
...
//        this.txtCod.grabFocus();
        ...
        if (res == JOptionPane.OK_OPTION) {
            try {
                if (botonagregar == true) {
                    ...
                    String sql = "INSERT INTO imputacion (imp_cod,imp_des)"
    ...
    private void txtCodActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtCodActionPerformed
        String sql1 = "SELECT * FROM imputacion WHERE  imp_cod=" + txtCod.getText();
        ...
        if (FrmPrincipal.isNumeric(txtCod.getText())) {
            if (validar(sql1) == true) {
                ...
                if (botonmodificar == true) {
                    try {
                        String sql = "Select * FROM imputacion "
                                + "WHERE imp_cod=" + txtCod.getText();
                        //System.out.println(sql);
                        rs = con.Consulta(sql);
                        rs.next();
                        txtNom.setText(rs.getString("imp_des"));
                        txtNom.setEnabled(true);
                        txtNom.grabFocus();
                    } catch (SQLException ex) {
                        Logger.getLogger(Imputacion.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void txtNomFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtNomFocusLost
        String sql = "SELECT * FROM imputacion WHERE imp_des='" + txtNom.getText() + "'";
    ...
    private void TablaCatedraMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TablaCatedraMouseClicked
        try {
            conexion();
            String sql = "Select * FROM imputacion "
                    + "WHERE imp_cod=" + TablaCatedra.getValueAt(TablaCatedra.getSelectedRow(), 0);
            System.out.println(sql);
            rs = con.Consulta(sql);
            rs.next();
            txtCod.setText(rs.getString("imp_cod"));
            txtNom.setText(rs.getString("imp_des"));
            con.CerrarConexion();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                Imputacion dialog = new Imputacion(new javax.swing.JDialog(), true);
